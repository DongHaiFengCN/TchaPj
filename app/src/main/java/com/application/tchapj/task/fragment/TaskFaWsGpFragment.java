...
public class TaskFaWsGpFragment extends BaseMvpFragment<IFaTaskView, FaTaskPresenter> implements IFaTaskView, ImagePickerAdapter.OnRecyclerViewItemClickListener {
...
//    FlowTagLayout fa_dygp_fl;
    ...
    private String taskType;    // 0 朋友圈 1 微博 2 抖音合拍 3 抖音原创 4其他 6微视合拍 7微视原创
...
/**
 对于支付结果，请商户依赖服务端的异步通知结果。同步通知结果，仅作为支付结束的通知。*/
    ...
    private AnimationDrawable animationDrawable;//上传图片/视频加载动画

    // 接收参数
    public static TaskFaWsGpFragment newInstance(String param) {
        Bundle args = new Bundle();
        args.putString("args", param);
        TaskFaWsGpFragment fragment = new TaskFaWsGpFragment();
    ...
    public int getRootViewId() {
        return R.layout.activity_fa_dygptask;//和抖音合拍用的同一个布局
    }
...
//        picker.setStepMinute(5);
        ...
        fa_dygp_bt.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                taskType = "6";      // 0 朋友圈 1 微博 2 抖音合拍 3 抖音原创 4其他
...
//                Fans = fa_dygp_fanset.getText().toString();           // 要求粉丝数量
               ...
               /* if (positionlist.size() == 1) {
                ...
                if(imagelist != null && imagelist.size() > 1){
                    taskImgUrl = imagelist.get(1);// 活动图片url或视频url
                }
                imagelist.clear();
        ...
        gridImageAdapter.setOnItemClickListener(new GridImageAdapter.OnItemClickListener() {
            ...
            public void onItemClick(int position, View v) {
                ...
                switch (mediaType) {
                    case 1:
                        // 预览图片
                        PictureSelector.create(TaskFaWsGpFragment.this).externalPicturePreview(position, selectList);
                        break;
                    case 2:
                        // 预览视频
                        PictureSelector.create(TaskFaWsGpFragment.this).externalPictureVideo(media.getPath());
                        break;
                    case 3:
                        // 预览音频
                        PictureSelector.create(TaskFaWsGpFragment.this).externalPictureAudio(media.getPath());
                        break;
                }
            }
        });
        ...
        public void onAddPicClick() {
            ...
            showDialog(new SelectDialog.SelectDialogListener() {
                ...
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    ...
                    if (position == 0) {
                        // 单独拍照
                        PictureSelector.create(TaskFaWsGpFragment.this)
