 ...
 */
// 抖音原创的领任务
public class TaskSquareDyYcActivity extends BaseMvpActivity<DailyTaskDouyinView, DailyTaskDouyinPresenter> implements DailyTaskDouyinView {
...
//    TextView releasefActivitysValueTv1;
    ...
    public int getRootViewId() {
        return R.layout.activity_task_douyin;
    }
...
//                releasefChannelValueTv1.setText("朋友圈");
                ...
                Glide.with(this).asBitmap().apply(options).load(R.drawable.quzi).into(new BitmapImageViewTarget(friendCircleImg) {
                    ...
                    protected void setResource(Bitmap resource) {
                        RoundedBitmapDrawable circularBitmapDrawable = RoundedBitmapDrawableFactory.create(TaskSquareDyYcActivity.this.getResources(), resource);
...
//                releasefChannelValueTv1.setText("抖音原创");
    ...
    public void onTaskDouyin(BaseBean baseBean) {
        if (!StringUtils.isNullOrEmpty(baseBean.getCode()) && baseBean.getCode().equals("000")) {
            Intent intent = new Intent(TaskSquareDyYcActivity.this, TaskSquareDyYcHzActivity.class);
            startActivity(intent);
            Toast.makeText(TaskSquareDyYcActivity.this, "操作成功", Toast.LENGTH_SHORT).show();
            finish();
        }else{
            if(!StringUtils.isNullOrEmpty(baseBean.getDescription())){
                Toast.makeText(TaskSquareDyYcActivity.this, baseBean.getDescription(), Toast.LENGTH_SHORT).show();
            }else{
                Toast.makeText(TaskSquareDyYcActivity.this, "操作失败", Toast.LENGTH_SHORT).show();
            }
        }
    ...
    private void showInfoDialog() {
        dialog = new MaterialDialog(TaskSquareDyYcActivity.this)
                .setTitle("提示")
