package com.application.tchapj.utils2.videoplayer;
...
public class NiceVideoPlayer extends FrameLayout
    ...
    public void setSpeed(float speed) {
        if (mMediaPlayer instanceof IjkMediaPlayer) {
            ...
            LogUtils.error("只有IjkPlayer才能设置播放速度");
        }
    }
    ...
    public void start() {
        if (mCurrentState == STATE_IDLE) {
            ...
            LogUtils.error("NiceVideoPlayer只有在mCurrentState == STATE_IDLE时才能调用start方法.");
        }
    }
    ...
    public void restart() {
        if (mCurrentState == STATE_PAUSED) {
            ...
            LogUtils.error("STATE_PLAYING");
        } else if (mCurrentState == STATE_BUFFERING_PAUSED) {
            mMediaPlayer.start();
            mCurrentState = STATE_BUFFERING_PLAYING;
            mController.onPlayStateChanged(mCurrentState);
            LogUtils.error("STATE_BUFFERING_PLAYING");
        } else if (mCurrentState == STATE_COMPLETED || mCurrentState == STATE_ERROR) {
            mMediaPlayer.reset();
            openMediaPlayer();
        } else {
            LogUtils.error("NiceVideoPlayer在mCurrentState == " + mCurrentState + "时不能调用restart()方法.");
        }
    }
    ...
    public void pause() {
        if (mCurrentState == STATE_PLAYING) {
            ...
            mController.onPlayStateChanged(mCurrentState);
            LogUtils.error("STATE_PAUSED");
        }
        if (mCurrentState == STATE_BUFFERING_PLAYING) {
            mMediaPlayer.pause();
            mCurrentState = STATE_BUFFERING_PAUSED;
            mController.onPlayStateChanged(mCurrentState);
            LogUtils.error("STATE_BUFFERING_PAUSED");
        }
    }
...
//                    ((IjkMediaPlayer)mMediaPlayer).setOption(4, "framedrop", 1L);
    ...
    private void openMediaPlayer() {
        ...
        try {
            ...
            mController.onPlayStateChanged(mCurrentState);
            LogUtils.error("STATE_PREPARING");
        } catch (IOException e) {
            e.printStackTrace();
            LogUtils.error("打开播放器发生错误", e);
        }
    }
    ...
    private IMediaPlayer.OnPreparedListener mOnPreparedListener
        ...
        public void onPrepared(IMediaPlayer mp) {
            ...
            mController.onPlayStateChanged(mCurrentState);
            LogUtils.error("onPrepared ——> STATE_PREPARED");
            mp.start();
            // 从上次的保存位置播放
    ...
    private IMediaPlayer.OnVideoSizeChangedListener mOnVideoSizeChangedListener
        ...
        public void onVideoSizeChanged(IMediaPlayer mp, int width, int height, int sar_num, int sar_den) {
            ...
            LogUtils.error("onVideoSizeChanged ——> width：" + width + "， height：" + height);
        }
    };
    ...
    private IMediaPlayer.OnCompletionListener mOnCompletionListener
        ...
        public void onCompletion(IMediaPlayer mp) {
            ...
            mController.onPlayStateChanged(mCurrentState);
            LogUtils.error("onCompletion ——> STATE_COMPLETED");
            // 清除屏幕常亮
    ...
    private IMediaPlayer.OnErrorListener mOnErrorListener
        ...
        public boolean onError(IMediaPlayer mp, int what, int extra) {
            ...
            if (what != -38 && what != -2147483648 && extra != -38 && extra != -2147483648) {
                ...
                mController.onPlayStateChanged(mCurrentState);
                LogUtils.error("onError ——> STATE_ERROR ———— what：" + what + ", extra: " + extra);
            }
            return true;
        }
    };
    ...
    private IMediaPlayer.OnInfoListener mOnInfoListener
        ...
        public boolean onInfo(IMediaPlayer mp, int what, int extra) {
            if (what == IMediaPlayer.MEDIA_INFO_VIDEO_RENDERING_START) {
                ...
                mCurrentState = STATE_PLAYING;
                mController.onPlayStateChanged(mCurrentState);
                LogUtils.error("onInfo ——> MEDIA_INFO_VIDEO_RENDERING_START：STATE_PLAYING");
            } else if (what == IMediaPlayer.MEDIA_INFO_BUFFERING_START) {
                // MediaPlayer暂时不播放，以缓冲更多的数据
                if (mCurrentState == STATE_PAUSED || mCurrentState == STATE_BUFFERING_PAUSED) {
                    mCurrentState = STATE_BUFFERING_PAUSED;
                    LogUtils.error("onInfo ——> MEDIA_INFO_BUFFERING_START：STATE_BUFFERING_PAUSED");
                } else {
                    mCurrentState = STATE_BUFFERING_PLAYING;
                    LogUtils.error("onInfo ——> MEDIA_INFO_BUFFERING_START：STATE_BUFFERING_PLAYING");
                }
                mController.onPlayStateChanged(mCurrentState);
                ...
                if (mCurrentState == STATE_BUFFERING_PLAYING) {
                    mCurrentState = STATE_PLAYING;
                    mController.onPlayStateChanged(mCurrentState);
                    LogUtils.error("onInfo ——> MEDIA_INFO_BUFFERING_END： STATE_PLAYING");
                }
                if (mCurrentState == STATE_BUFFERING_PAUSED) {
                    mCurrentState = STATE_PAUSED;
                    mController.onPlayStateChanged(mCurrentState);
                    LogUtils.error("onInfo ——> MEDIA_INFO_BUFFERING_END： STATE_PAUSED");
                }
            } else if (what == IMediaPlayer.MEDIA_INFO_VIDEO_ROTATION_CHANGED) {
                // 视频旋转了extra度，需要恢复
                if (mTextureView != null) {
                    mTextureView.setRotation(extra);
                    LogUtils.error("视频旋转角度：" + extra);
                }
            } else if (what == IMediaPlayer.MEDIA_INFO_NOT_SEEKABLE) {
                LogUtils.error("视频不能seekTo，为直播视频");
            } else {
                LogUtils.error("onInfo ——> what：" + what);
            }
            return true;
        }
    };
    ...
    public void enterFullScreen() {
        ...
        LogUtils.error("MODE_FULL_SCREEN");
    }
    ...
    public void enterTinyWindow() {
        ...
        mController.onPlayModeChanged(mCurrentMode);
        LogUtils.error("MODE_TINY_WINDOW");
    }
