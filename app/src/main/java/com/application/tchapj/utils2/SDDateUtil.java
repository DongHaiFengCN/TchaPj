...
public class SDDateUtil {
    ...
    public static String formatDuringFrom(long timestamp) {
        ...
        try {
            curDate = dateformat.parse(mil2yyyyMMdd(timestamp));
        } catch (ParseException e) {
            e.printStackTrace();
        }
        long diff = (endDate.getTime() - curDate.getTime());
        long between = diff / (1000L * 24L * 3600L);
        ...
        if (timeSpan <= 0 || timeSpan < MILLISECONDS_MINUTES) {
            ...
            } else if((int)between>=7){
                return "一周前";
            }else {
                return String.valueOf(between) + "天前";
            }
        }
    ...
    public static String formatDuringFrom2(long timestamp) {
        ...
        try {
            endDate = dateformat.parse(mil2yyyyMMdd(timestamp));
        } catch (ParseException e) {
            e.printStackTrace();
        }
        ...
        long current = System.currentTimeMillis();
        long timeSpan = timestamp - current; // 分钟

        long hhmm = getDuringHours(timeSpan);

        long min = getDuringMinutes(timeSpan);

        return between+ "天"+hhmm+"小时"+min+"分钟";
